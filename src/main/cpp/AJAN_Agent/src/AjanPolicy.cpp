...
namespace despot {
        ...
        DefaultPolicy(model,bound) {cout<<"AjanPolicy Created"<<endl;}
...
//        }
        ...
        ACT_TYPE AjanPolicy::Action(const vector<State*>& particles, RandomStreams& streams,
            ...
            // TODO: Implement the java connection
//            jint val = getEnv()->CallIntMethod(getAjanPolicyObject(),
//                                    getMethodID("Policy","TestMethod"));
//            cout<<"Value"<<val<<endl;
            return getEnv()->CallIntMethod(getAjanPolicyObject(),
                                           getMethodID("Policy","Action"),
                                           getAjanAgentStateVectorFromStateVector(particles),
                                           reinterpret_cast<jlong>(&history));
            //region
//            if(history.Size() == 0) {
