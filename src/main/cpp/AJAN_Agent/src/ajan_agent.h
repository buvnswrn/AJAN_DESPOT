...
namespace despot {
    ...
    class AJANAgentState:public State {
        public:
            ...
            AJANAgentState(jobject stateObject);
    ...
    public:
        ...
        bool
        getAJANStep(State &s, double random_num, ACT_TYPE action, double &reward, OBS_TYPE &obs, const char *methodName,
                    const char *returnType) const;

        jobject convertToAJANAgentState(const State &state) const;
        ...
        void UpdateValues(State &state, double &reward, OBS_TYPE &obs) const;

        void UpdateStateValues(State &state, jobject pJobject) const;
    };
}
